...
type ILogger = abstract Log : LogLevels -> Printf.StringFormat<'a,unit> -> 'a
...
//      Printf.kprintf (printfn "[%s][%A] %s" (LevelToString level) System.DateTime.Now) format
      Printf.kprintf (printfn "[%s][%A] %s" (LoglevelToString level) System.DateTime.Now) format
 }
